 ...
 * Tests the complete error handling flow from controller to global exception
 * handler.
 */
@SpringBootTest
...
class ReservationSearchErrorHandlingTest {
        ...
        void searchValidationError_shouldReturnProperErrorResponse() throws Exception {
                ...
                given(reservationService.listWithFilters(
                                isNull(), isNull(), isNull(), isNull(), isNull(), isNull(), eq(shortSearchTerm),
                                any(Pageable.class)))
                                .willThrow(new SearchValidationException(shortSearchTerm,
                ...
                mockMvc.perform(get("/api/reservations")
                                ...
                                // .andExpect(jsonPath("$.type").value("about:blank")) // Default type is
                                // omitted in Spring Boot 4
                                ...
                                .andExpect(jsonPath("$.validationError")
                                                .value("Search term must be at least 2 characters long"))
        ...
        void searchValidationErrorTooLong_shouldReturnProperErrorResponse() throws Exception {
                ...
                given(reservationService.listWithFilters(
                                isNull(), isNull(), isNull(), isNull(), isNull(), isNull(), eq(longSearchTerm),
                                any(Pageable.class)))
                                .willThrow(new SearchValidationException(longSearchTerm,
        ...
        void searchValidationErrorInvalidChars_shouldReturnProperErrorResponse() throws Exception {
                ...
                given(reservationService.listWithFilters(
                                isNull(), isNull(), isNull(), isNull(), isNull(), isNull(), eq(invalidSearchTerm),
                                any(Pageable.class)))
                                .willThrow(new SearchValidationException(invalidSearchTerm,
                                                "Search term contains only invalid characters"));
                ...
                mockMvc.perform(get("/api/reservations")
                                ...
                                .andExpect(jsonPath("$.searchRequirements.allowedCharacters")
                                                .value("alphanumeric, spaces, hyphens, dots, @ symbols, underscores, plus signs, parentheses"));
        }
        ...
        void regularIllegalArgumentException_shouldReturnBadRequest() throws Exception {
                // Given
                given(reservationService.listWithFilters(
                                isNull(), isNull(), isNull(), isNull(), isNull(), isNull(), eq("test"),
                                any(Pageable.class)))
                                .willThrow(new IllegalArgumentException(
                                                "Invalid date range: endDate must be on/after startDate"));
                ...
                mockMvc.perform(get("/api/reservations")
                                ...
                                .andExpect(jsonPath("$.detail")
                                                .value("Invalid date range: endDate must be on/after startDate"))
                                .andExpect(jsonPath("$.searchTerm").doesNotExist()) // Should not have search-specific
                                                                                    // properties
        ...
        void validSearch_shouldReturnResults() throws Exception {
                ...
                given(reservationService.listWithFilters(
                                isNull(), isNull(), isNull(), isNull(), isNull(), isNull(), eq(validSearchTerm),
                                any(Pageable.class)))
